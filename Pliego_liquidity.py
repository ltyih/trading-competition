...
EPS = 1e-9
DEBUG_BOOK = False
REPORT_ONLY_MODE = True  # True = monitor/report tenders only (no trading actions)
DISABLE_TENDER_PRICE_SUBMISSION = True  # Never include `price` when accepting tenders


def normalize_price(raw_price: Any) -> float:
    """Normalize API prices that may be returned in cents (e.g., 2469 -> 24.69)."""
    try:
        px = float(raw_price)
    ...
    # LT3 tickers are priced in tens, so 4+ digit whole prices are cents.
    if px >= 1000 and abs(px - round(px)) < EPS:
        return px / 100.0
    return px


def normalize_order_book_prices(order_book: dict) -> dict:
    """Return a copy of the order book with normalized level prices."""
    normalized = {'bid': [], 'ask': []}
    for side in ('bid', 'ask'):
        for level in order_book.get(side, []):
            lvl = dict(level)
            lvl['price'] = normalize_price(lvl.get('price', 0.0))
            normalized[side].append(lvl)
    return normalized
...
def get_order_book(session, ticker: str, limit: int = BOOK_LIMIT) -> dict:
    ...
    if isinstance(book, dict) and ('bids' in book or 'asks' in book):
        ...
        return normalize_order_book_prices(normalized)
...
def is_winner_take_all_tender(tender: dict) -> bool:
    """Best-effort detection for winner-take-all tenders across API variants."""
    bool_like_keys = (
        'is_winner_take_all',
        'winner_take_all',
        'winner_takes_all',
    ...
    for key in bool_like_keys:
        ...
        if isinstance(v, str):
            ...
            if s in {'1', 'TRUE', 'YES', 'WTA',
                     'WINNER_TAKE_ALL', 'WINNER_TAKES_ALL'}:
                return True

    text_keys = (
        'tender_type', 'type', 'kind', 'mechanism',
        'auction_type', 'description'
    )
    for key in text_keys:
        s = str(tender.get(key, '')).strip().upper()
        if not s:
            continue
        ...
        if ('WINNER' in normalized
                and 'TAKE' in normalized
                and 'ALL' in normalized):
            return True

    return False


def is_auction_tender(tender: dict) -> bool:
    """Returns True when the tender is competitive/auction."""
    return not bool(tender.get('is_fixed_bid', not FORCE_COMPETITIVE))


def accept_tender(session, tender_id: int, price: float = None) -> bool:
    """Accept tender without any submitted price payload."""
    if REPORT_ONLY_MODE:
        return False
    resp = session.post(f'{BASE_URL}/tenders/{int(tender_id)}', params={})
    return resp.ok
...
class LiquidityAnalyzer:
    ...
    def calculate_price_for_confidence(
        self,
        tender: dict,
        order_book: dict,
        target_confidence: float = 0.75,
    ) -> Optional[float]:
        """Find an auction price that yields at least target confidence.

        Returns:
        - BUY tender: highest price that still meets target confidence.
        - SELL tender: lowest price that still meets target confidence.
        - None if target confidence is not reachable with current depth/costs.
        """
        tender_action = str(tender.get('action', '')).upper()
        target_confidence = float(max(0.0, min(0.95, target_confidence)))
        ...
        def meets(px: float) -> bool:
            ev = self.evaluate_tender_offer(
                tender, order_book, override_price=px)
            return (ev.get('decision') == 'ACCEPT'
                    and ev.get('confidence', 0.0) >= target_confidence)

        # BUY tender: lower price improves economics; find highest acceptable px.
        if tender_action == 'BUY':
            lo = 0.01
            hi = max(0.01, ref_px)
            if not meets(lo):
        ...
        # SELL tender: higher price improves economics; find lowest acceptable px.
        lo = max(0.01, ref_px)
        hi = max(lo + 0.01, lo * 1.5)

        if meets(lo):
            return round(lo, 2)

        max_hi = max(lo * 100.0, lo + 1000.0)
        while hi < max_hi and not meets(hi):
            hi *= 1.5
    ...
    def evaluate_tender_offer(self, tender: dict, order_book: dict,
        ...
        tender_action = str(tender.get('action', '')).upper()
        tender_price = (normalize_price(override_price)
                        if override_price is not None
                        else normalize_price(tender.get('price', 0.0)) / 100)
        quantity = int(tender.get('quantity', 0))
    ...
    with requests.Session() as session:
        ...
        while tick < 600 and not shutdown:
            try:
                ...
                # 2) Position limits (computed for safety checks; no per-tick print)
                ...
                for tender in tenders:
                    ...
                    # Winner-take-all tenders are strictly ignored.
                    if is_winner_take_all_tender(tender):
                        summary = (
                            f"id={tender_id}|t={ticker}"
                            f"|a={tender_action}|q={tender_qty}"
                            f"|exp={tender_exp}|wta_ignored"
                        )
                        if last_tender_summary.get(tender_id) != summary:
                            print("\n" + "!" * 80)
                            print(
                                f"[WINNER-TAKES-ALL TENDER] "
                                f"Tender #{tender_id} | {ticker} | "
                                f"{tender_action} | Qty: {tender_qty:,} | "
                                f"Expires(tick): {tender_exp}")
                            print(
                                "Action: ignored "
                                "(script does not touch WTA tenders)")
                            print("!" * 80)
                            last_tender_summary[tender_id] = summary
                        continue

                    # Competitive/auction tenders are strictly ignored.
                    if is_auction_tender(tender):
                        continue
                    ...
                    if last_tender_summary.get(tender_id) != summary:
                        print("\n" + "!" * 80)
                        print(
                            f"[FIXED TENDER] Tender #{tender_id} | "
                    ...
                    if (not REPORT_ONLY_MODE
                            and evaluation['decision'] == 'ACCEPT'
                            and evaluation['confidence'] > 0.5):
                        # Fixed tenders only: accept without submitting a price
                        ok = accept_tender(
                            session, tender_id, price=None)

                        if ok:
                            eff_price = normalize_price(
