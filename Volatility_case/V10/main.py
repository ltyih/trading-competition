...
Strategy:
  ...
  - Delta hedge on a configurable sub-second cadence between ticks
"""
...
from config import CASE_POLL_INTERVAL_SEC, HEDGE_INTERVAL_SEC, TICKS_PER_SUBHEAT
...
def run_trading_loop(api: RITApi, engine: StraddleEngine):
    last_tick = -1
    cycle_count = 0
    poll_interval = max(float(CASE_POLL_INTERVAL_SEC), 0.01)
    hedge_interval = max(float(HEDGE_INTERVAL_SEC), 0.01)
    next_hedge_due = time.monotonic() + hedge_interval
    ...
    while True:
        ...
        if tick != last_tick:
            ...
            # Align hedge-only cadence after each full tick-cycle.
            next_hedge_due = time.monotonic() + hedge_interval
        elif now >= next_hedge_due:
            engine.execute_hedge_cycle(tick)
            now_after = time.monotonic()
            while next_hedge_due <= now_after:
                next_hedge_due += hedge_interval
